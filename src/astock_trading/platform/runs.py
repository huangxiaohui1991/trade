...
from astock_trading.platform.time import local_date_bounds_utc, local_now, utc_now
...
class RunRepository:
    """Repository for pipeline run lifecycle and idempotency."""
    ...
    def fail_run(self, run_id: str, error: str, artifacts: Optional[dict] = None) -> None:
        """Mark a run as failed."""
        self._conn.execute(
            """UPDATE run_log
               SET status = 'failed', finished_at = ?, error_message = ?, artifacts_json = ?
               WHERE run_id = ?""",
            (
                utc_now_iso(),
                error[:4000],
...
class RunJournal(RunRepository):
    """Backward-compatible run journal facade."""
