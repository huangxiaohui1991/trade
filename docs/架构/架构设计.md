...
### Agent 直接查询命令（data / score / risk / market / shadow）

不在 workflow 内时，Agent 可直接调用以下命令查询数据：

```bash
# 数据查询
bin/trade data technical <code> [--days N]   # 均线、RSI、动量
bin/trade data financial <code>               # ROE、营收增长、现金流
bin/trade data flow <code> [--days N]         # 主力资金流、北向
bin/trade data realtime <code> [code...]      # 实时价格、涨跌幅
bin/trade data market-index                   # 大盘指数 MA20/MA60 信号

# 评分
bin/trade score single <code> [--name NAME]  # 单股四维评分
bin/trade score batch --codes <c1,c2>         # 批量评分（代码列表）
bin/trade score batch --pool core|watch|all   # 批量评分（池子）
bin/trade score pool --pool core|watch|all    # 对整个池子评分

# 风控
bin/trade risk check <code> [--name NAME]     # 黑名单 + ST/涨停/流动性/PE 检查
bin/trade risk portfolio --exposure N --week-buys N --holding-count N [--proposed N]
bin/trade risk position-size --capital N --price N [--risk-pct N]
bin/trade risk stop-loss --cost N [--ma20 N] [--style N] [--highest-price N]
bin/trade risk should-exit <code> --price N [--ma20 N] [--highest-price N]

# 市场时钟
bin/trade market signal                       # GREEN / YELLOW / RED / CLEAR
bin/trade market detail                       # 各指数详细数据
bin/trade market multiplier                   # 仓位系数

# 模拟盘
bin/trade shadow status                       # 模拟盘总状态
bin/trade shadow check-stops [--dry-run]      # 止损/止盈信号检查
bin/trade shadow buy-new-picks [--dry-run]   # 买入新标的
bin/trade shadow reconcile [--apply] [--window N]  # 对账
bin/trade shadow consistency [--window N]     # 事件流与券商状态一致性
bin/trade shadow report                       # Obsidian 报告
```
...
- [x] Agent 直接查询 CLI（data / score / risk / market / shadow 子命令组）
